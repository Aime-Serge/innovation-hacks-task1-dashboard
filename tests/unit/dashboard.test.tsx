...
describe("TC-001 dashboard content (FR-01..04)", () => {
  ...
  it("TC-070 shows skeletons while loading", async () => {
    await open("loading");
...
describe("TC-003 dashboard lists", () => {
  it("TC-003 deadline rows show title, project, priority and date", () => {
    ...
    const task = makeTask({ title: "Ship it", projectId: "p-x", priority: "urgent", dueDate: "2030-01-12" });
  ...
  it("TC-003 activity rows read as sentences with relative times", () => {
    const items = [
      { id: "a1", actorId: "u1", projectId: "p1", type: "completed" as const, at: "2030-01-10T10:00:00.000Z" },
      { id: "a2", actorId: "ghost", projectId: "ghost", type: "created" as const, at: "2030-01-09T10:00:00.000Z" },
    ];
    renderApp(
      <ActivityFeed
        items={items}
        users={new Map([["u1", { id: "u1", name: "Ada", email: "a@b.co", role: "lead", preferences: { theme: "system" } }]])}
