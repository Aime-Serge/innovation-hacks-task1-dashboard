...
function AvatarSection() {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError(err instanceof MockAuthError ? err.message : "Something went wrong. Please try again.");
    } finally {
      setUploading(false);
    }
  }
...
function ChangePasswordSection() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(
        err instanceof MockAuthError ? err.message : "Something went wrong. Please try again.",
      );
    } finally {
      setSubmitting(false);
    }
...
function DeleteAccountSection() {
  ...
  return (
    <SectionCard
      ...
      description="Permanently deletes your mock account. This can't be undone."
    >
      <button
        type="button"
      ...
      {confirming && (
        <ConfirmDialog
          title="Delete account"
          message={`Delete your account "${user.email}"? This can't be undone.`}
...
export function SettingsView() {
  // Gate on auth status rather than letting each section handle "user is
  // still null" itself: ProfileSection's useState(user.name) initializer
  // only runs on a component's *first* mount — if that first mount
  // happens while the session is still resolving, the name/email fields
  // would stay stuck empty forever, even after the real user data
  // arrives moments later. Mounting these sections only once status is
  // "authenticated" guarantees their first mount already has real data.
